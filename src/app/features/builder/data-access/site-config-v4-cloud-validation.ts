...
import siteConfigV4Schema from '../../../../../contracts/site-config/v4.schema.json';
import type {
  SiteConfigV4ValidationErrorCode,
  SiteConfigV4ValidationResult,
} from './site-config-v4-cloud-validation.types';
...
const schema = siteConfigV4Schema as object;
const validateSchema: ValidateFunction = new Ajv({
  allErrors: false,
  strict: true,
}).compile(schema);

export function validateAndCanonicalizeCloudSiteConfigV4Json(
  serialized: string,
): SiteConfigV4ValidationResult {
  if (UTF8_ENCODER.encode(serialized).byteLength > SITE_CONFIG_V4_JSON_ENVELOPE_BYTES_LIMIT) {
  ...
  const canonicalValue = canonicalizeBundledMediaPaths(parsed) as Readonly<Record<string, unknown>>;
  const canonicalJson = stableJsonStringify(canonicalValue);
  const canonicalBytes = UTF8_ENCODER.encode(canonicalJson).byteLength;
...
function failure(code: SiteConfigV4ValidationErrorCode): SiteConfigV4ValidationResult {
...
function hasValidSiteConfigSemantics(siteConfig: Record<string, unknown>): boolean {
  ...
  if (!addUniqueString(blockIds, header['id']) || !addUniqueString(blockIds, footer['id'])) {
    return false;
  }
  if (
  ...
  for (const pageValue of pages) {
    ...
    if (!addUniqueString(pageIds, pageId) || !addUniqueString(pageSlugs, slug)) return false;
    ...
    for (const blockValue of blocks) {
      ...
      if (blockValue['type'] === 'siteHeader' || blockValue['type'] === 'siteFooter') return false;
      if (
  ...
  return visitRecords(siteConfig, (record) => {
    ...
    if (
      ...
      (!isSafeCloudLinkTarget(record['embedUrl']) || !isCredentialFreeHttpsUrl(record['embedUrl']))
    ) {
      return false;
    }

    if (typeof record['buttonHref'] === 'string' && !isSafeCloudLinkTarget(record['buttonHref'])) {
      return false;
    }
...
function hasUniqueIdsInContainingArrays(value: unknown): boolean {
  if (Array.isArray(value)) {
    ...
    if (recordsWithIds.length > 0) {
      const ids = new Set(recordsWithIds.map((record) => record['id'] as string));
      if (ids.size !== recordsWithIds.length) return false;
    }
    return value.every(hasUniqueIdsInContainingArrays);
  }

  return !isRecord(value) || Object.values(value).every(hasUniqueIdsInContainingArrays);
}

function hasUniqueContainedIds(block: Record<string, unknown>): boolean {
  const collectionTypes = new Set(['featureGrid', 'offerList', 'gallery', 'testimonials', 'faq']);
  const values =
    block['type'] === 'leadForm'
      ? block['fields']
  ...
  return values.every((value) => isRecord(value) && addUniqueString(ids, value['id']));
}
...
function isSafeCloudMediaSource(source: string): boolean {
  ...
  if (!source.startsWith('images/') && !source.startsWith('./images/')) return false;
  if (
    source.includes('\\') ||
    source.includes('?') ||
    source.includes('#') ||
...
function isSafeCloudLinkTarget(target: string): boolean {
  ...
  if (target.startsWith('mailto:')) return /^mailto:[^\s@]+@[^\s@]+$/u.test(target);
  if (target.startsWith('tel:')) return /^tel:\+?[0-9()\-\s]{3,32}$/u.test(target);
  return false;
}
...
function visitRecords(
  ...
  if (Array.isArray(value)) return value.every((item) => visitRecords(item, visitor));
...
function canonicalizeBundledMediaPaths(value: unknown): unknown {
  ...
  const entries = Object.entries(value).map(([key, item]) => {
    if (key === 'src' && typeof item === 'string' && item.startsWith('./images/')) {
